...
def _real_main(argv=None):
    ...
    parser, opts, args = my_parseOpts(argv)
    ...
    is_custom = opts.get_call_custom
    if opts.get_instagram_cookie_str:
        ydl_opts.update({'instagram_cookie': opts.get_instagram_cookie_str.strip()})
    with (MyYoutubeDL(ydl_opts) if is_custom else YoutubeDL(ydl_opts)) as ydl:
        # Update version
        if opts.update_self:
