...
class CustomBandcampIE(InfoExtractor):
...
class CustomBandcampAlbumIE(CustomBandcampIE):
    IE_NAME = 'CustomBandcamp:album'
    ...
    def suitable(cls, url):
        return (False
                if CustomBandcampWeeklyIE.suitable(url) or CustomBandcampIE.suitable(url)
                else super(CustomBandcampAlbumIE, cls).suitable(url))

    def _real_extract(self, url):
        uploader_id, album_id = re.match(self._VALID_URL, url).groups()
        playlist_id = album_id or uploader_id
        webpage = self._download_webpage(url, playlist_id)
        thumbnail = self._html_search_meta('og:image', webpage, 'image', default='')
        ...
        for t in track_info:
            if t.get('duration'):
                info = {
                    ...
                    'id': str_or_none(t.get('track_id') or t.get('id')),
                    'ie_key': CustomBandcampIE.ie_key(),
                    'thumbnail': thumbnail,
                    'player': f'''<audio controls="controls" style="position:
                                  relative;margin: 0 auto;display:
                                  table;z-index: 10;height: 80px;" 
                                 src="{t['file']['mp3-128']}" autoplay=""></audio>'''
                }
                entries.append(info)

        # entries = [
        #     self.url_result(
        #         urljoin(url, t['title_link']), CustomBandcampIE.ie_key(),
...
class CustomBandcampWeeklyIE(CustomBandcampIE):
    IE_NAME = 'CustomBandcamp:weekly'
