...
class CustomArteTVBaseIE(InfoExtractor):
    _ARTE_LANGUAGES = 'fr|de|en|es|it|pl'
    _API_BASE = 'https://api.arte.tv/api/player/v1'


class CustomArteTVIE(CustomArteTVBaseIE):
    _VALID_URL = r'''(?x)
                    https?://
                        (?:
                        ...
                        /(?P<id>\d{6}-\d{3}-[AF])
                    ''' % {'langs': CustomArteTVBaseIE._ARTE_LANGUAGES}
    _TESTS = [{
    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            'title': title,
            # we add duration
            'duration': player_info.get('videoDurationSeconds'),
...
class CustomArteTVEmbedIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        video_id = CustomArteTVIE._match_id(json_url)
        return self.url_result(
            json_url, ie=CustomArteTVIE.ie_key(), video_id=video_id)


class CustomArteTVPlaylistIE(CustomArteTVBaseIE):
    _VALID_URL = r'https?://(?:www\.)?arte\.tv/(?P<lang>%s)/videos/(?P<id>RC-\d{6})' % CustomArteTVBaseIE._ARTE_LANGUAGES
    _TESTS = [{
