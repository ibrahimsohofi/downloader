...
class CustomTumblrIE(InfoExtractor):
    _VALID_URL = r'https?://(?P<blog_name>[^/?#&]+)?\.(?:tumblr\.com|luisonte.es)/(?:post|video)/(?P<id>[0-9]+)(?:$|[/?#])'
    ...
    def _real_extract(self, url):
        ...
        video_url = self._html_search_meta('og:video', webpage, 'og:video', default='')
        if not video_url:
            raise ExtractorError(r'Could not find video url!')
        height = self._html_search_meta('og:video:height', webpage, 'og:video:height')
