...
class CustomBiliBiliIE(InfoExtractor):
    _VALID_URL = r'''(?x)
                    https?://
                        (?:(?:www|bangumi)\.)?
                        bilibili\.(?:tv|com)/
    ...
    def _real_extract(self, url):
        ...
        info = {
            ...
            'timestamp': timestamp,
            'thumbnail': thumbnail,
            'duration': float_or_none(playinfo['data']['dash']['duration']),
        }
        ...
        for audio in audios[:1]:
            audio_info = {
                'url': (audio.get('baseUrl') or audio['base_url']) + '&amoysharetype={"headers": {"Referer": "%s"}}' % url,
                'filesize': None,
                'format_id': '0',
                'format': '0 - unknown',
                'ext': 'm4a',
            ...
            formats.append(audio_info)

        videos = playinfo['data']['dash']['video']
        for video in videos:
            video_info = {
                'url': (video.get('baseUrl') or video['base_url']) + '&amoysharetype={"headers": {"Referer": "%s"}}' % url,
                'protocol': 'https',
                'ext': 'mp4',
                'height': video['height'],
                'filesize': None
...
class CustomBiliBiliBangumiIE(InfoExtractor):
    ...
    def suitable(cls, url):
        return False if CustomBiliBiliIE.suitable(url) else super(CustomBiliBiliBangumiIE, cls).suitable(url)
    ...
    def _real_extract(self, url):
        ...
        entries = [{
            ...
            'ie_key': CustomBiliBiliIE.ie_key(),
...
class CustomBilibiliAudioBaseIE(InfoExtractor):
...
class CustomBilibiliAudioIE(CustomBilibiliAudioBaseIE):
    ...
    def _real_extract(self, url):
        ...
        formats = [{
            'url': play_data['cdns'][0] + '&amoysharetype={"headers": {"Referer": "%s"}}' % url,
...
class CustomBilibiliAudioAlbumIE(CustomBilibiliAudioBaseIE):
    ...
    def _real_extract(self, url):
        ...
        for song in songs:
            ...
            entries.append(self.url_result(
                ...
                CustomBilibiliAudioIE.ie_key(), sid))
...
class CustomBiliBiliPlayerIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        return self.url_result(
            ...
            ie=CustomBiliBiliIE.ie_key(), video_id=video_id)
