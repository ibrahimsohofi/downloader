...
class CustomMixcloudBaseIE(InfoExtractor):
    @staticmethod
    def url_result(url, ie=None, video_id=None, video_title=None,
                   video_thumbnail=None, video_duration=None, video_uri=None):
        ...
        if video_thumbnail is not None:
            video_info['thumbnail'] = video_thumbnail
        else:
            video_info['thumbnail'] = ''
        player = f'''<iframe width="100%" height="100%" frameborder="0" allowfullscreen src="https://www.mixcloud.com/widget/iframe/?hide_cover=1&feed={video_uri}"></iframe>'''
        video_info['player'] = player
        return video_info
...
class CustomMixcloudIE(CustomMixcloudBaseIE):
    ...
    tags {
      tag {
        ...
        uri = '/%s/%s/' % (username, slug)
        player = f'''<iframe width="100%" height="100%" frameborder="0" allowfullscreen src="https://www.mixcloud.com/widget/iframe/?hide_cover=1&feed={uri}"></iframe>'''
...
class CustomMixcloudPlaylistBaseIE(CustomMixcloudBaseIE):
      ...
      pageInfo {
        ...
        hasNextPage
            ...
            for edge in items.get('edges', []):
                ...
                title = try_get(cloudcast, lambda x: x['name'], compat_str)
                thumbnail = cloudcast['picture']['urlRoot']
                thumbnail = 'https://thumbnailer.mixcloud.com/unsafe/360x360/' + thumbnail
                duration = cloudcast['audioLength']
                uri = '/%s/%s/' % (owner_username, slug)
                entries.append(self.url_result(
                    cloudcast_url, CustomMixcloudIE.ie_key(), video_id, title, thumbnail, duration, uri))
...
# class CustomMixcloudUserIE(CustomMixcloudPlaylistBaseIE):
#     _VALID_URL = r'https?://(?:www\.)?mixcloud\.com/(?P<id>[^/]+)/(?P<type>uploads|favorites|listens|stream)?/?$'
#     IE_NAME = 'custom mixcloud:user'
#
#     _TESTS = [{
...
class CustomMixcloudPlaylistIE(CustomMixcloudPlaylistBaseIE):
    _VALID_URL = r'https?://(?:www\.)?mixcloud\.com/(?P<user>[^/]+)/playlists/(?P<playlist>[^/]+)/?$'
    IE_NAME = 'custom mixcloud:playlist'
