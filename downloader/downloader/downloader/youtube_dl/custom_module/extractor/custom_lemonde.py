...
class CustomLemondeIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        embed_url = self._html_search_meta(
            'og:video', webpage, 'og video', default=''
        )
        if embed_url:
            if 'http:https:' in embed_url:
                embed_url = embed_url.replace('http:https:', 'https:')
            if 'dailymotion.com/embed' in embed_url:
                return self.url_result(embed_url, 'Dailymotion')
            elif 'youtube.com/embed' in embed_url:
                return self.url_result(embed_url, 'CustomYoutube')
