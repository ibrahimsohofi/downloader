...
class CustomNiconicoIE(InfoExtractor):
    IE_NAME = 'custom niconico'
    IE_DESC = 'ニコニコ動画'
    ...
    def _extract_format_for_quality(self, api_data, video_id, audio_quality, video_quality):
        ...
        video_url = session_response['data']['session']['content_uri']
        return {
            'url': video_url,
            'format_id': format_id,
            'ext': 'm3u8' if '.m3u8' in video_url else 'mp4',  # Session API are used in HTML5, which always serves mp4
    ...
    def _real_extract(self, url):
        ...
        # Just change extractor process, Since december 2020
        # Office site has`t fix program, So we maintain this
        ...
        dmc_info = api_data['media']['delivery']
        # "New" HTML5 videos
        quality_info = dmc_info['movie']
        for audio_quality in quality_info['audios']:
            for video_quality in quality_info['videos']:
        ...
        thumbnail = (
            api_data['video']['thumbnail']['player']
            or get_video_info(['thumbnail_url', 'thumbnailURL'])
        ...
        timestamp = (
            parse_iso8601(api_data['video'].get('registeredAt'))
            or parse_iso8601(get_video_info('first_retrieve'))
        ...
        view_count = int_or_none(get_video_info(['view_counter', 'viewCount'])) or api_data['video']['count'].get('view')
        if not view_count:
        ...
        return {
            ...
            'player': f'<iframe width="100%" height="100%" frameborder="0" allowfullscreen src="https://embed.nicovideo.jp/watch/{video_id}?autoplay=1"></iframe>'
        }
