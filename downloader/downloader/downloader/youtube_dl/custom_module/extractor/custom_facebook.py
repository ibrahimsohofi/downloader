...
class CustomFacebookInfoExtractor(InfoExtractor):

    def _real_extract(self, url, facebook_cookie):
        """Real extraction process. Redefine in subclasses."""
        pass

    def extract(self, url, facebook_cookie=''):
        """Extracts URL information and returns it in list of dicts."""
        try:
            for _ in range(2):
                try:
                    self.initialize()
                    ie_result = self._real_extract(url, facebook_cookie)
...
class CustomFacebookIE(CustomFacebookInfoExtractor):
    _VALID_URL = r'''(?x)
                    (?:
                        https?://
                    ...
                    (?P<id>[0-9]+)
                    '''
    _VALID_URL1 = r'^https?://(?:www\.)?fb\.watch/(?P<id>[^/]+)'
    ...
    def _extract_from_url(self, url, video_id, facebook_cookie):
        if re.match(self._VALID_URL1, url) is not None:
            head_req = HEADRequest(url)
            if facebook_cookie:
                headers = {'cookie': facebook_cookie}
                head_req.headers = headers
            ...
            if head_response is not False:
                ...
                new_url = new_url.replace('https://www.facebook.com/login/?next=', '')
                new_url = compat_urllib_parse_unquote(new_url)
                if re.match(self._VALID_URL, new_url) is not None:
                    url = new_url
                else:
                    raise ExtractorError(f'You Maybe Require Login To Pass Cookie: {head_response.geturl()}')
            else:
            ...
            if data:
                ...
                def parse_graphql_video(video):
                    ...
                    info = {
                        'id': v_id,
                        'formats': formats,
                        'thumbnail': try_get(video, lambda x: x['thumbnailImage']['uri']) or try_get(video, lambda x: x['preferred_thumbnail']['image']['uri']) or try_get(video, lambda x:  x['image_for_warning_screen']['uri']),
        ...
        if not video_title:
            video_title = self._html_search_meta(
                ['og:title', 'twitter:title', 'description'], webpage, 'title', default=None)
        if video_title:
    ...
    def _real_extract(self, url, facebook_cookie):
        ...
        return self._extract_from_url(real_url, video_id, facebook_cookie)


class CustomFacebookStoriesIE(CustomFacebookInfoExtractor):
    IE_NAME = 'facebook:stories'
    _VALID_URL = r'^https?://(?:www\.)?facebook\.com/stories/(?P<id>[0-9]+)/(?P<mid>[^/]+)?'
    ...
    def _real_extract(self, url, facebook_cookie):
        ...
        items = re.findall(r'"image":(\{.*?\})', webpage, re.S)
        if not items:
            raise ExtractorError(f'Can`t found items try other regex')
        formats = []
        for item in items:
        ...
        uploader = self._html_search_regex([r'"short_name":"(.*?)"', r'"name":"(.*?)"'], webpage, 'uploader', default='')
        if not uploader:
            raise ExtractorError(f'Can`t found uploader try other regex')
        result = {
            'type_name': 'GraphImage',
            'id': video_id,
            'formats': formats,
            'title': f'Image by {uploader}',
            'description': '',
            'thumbnail': thumbnail,
            'publishedAt': '',
        }
...
class CustomFacebookPluginsVideoIE(InfoExtractor):
    ...
    _TESTS = [{
        ...
        'add_ie': [CustomFacebookIE.ie_key()],
    }, {
