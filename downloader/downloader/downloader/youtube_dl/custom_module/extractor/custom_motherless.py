...
class CustomMotherlessIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        upload_date = self._html_search_regex(
            (r'class=["\']count[^>]+>(\d+\s+[a-zA-Z]{3}\s+\d{4})<',
             r'<strong>Uploaded</strong>\s+([^<]+)<',
             r'<span class="count">(.*?ago)</span>'), webpage, 'upload date')
...
class CustomMotherlessGroupIE(InfoExtractor):
    ...
    def suitable(cls, url):
        return (False if CustomMotherlessIE.suitable(url)
                else super(CustomMotherlessGroupIE, cls).suitable(url))
    ...
    def _extract_entries(self, webpage, base):
        ...
        for mobj in re.finditer(
            ...
            if not CustomMotherlessIE.suitable(video_url):
                continue
            video_id = CustomMotherlessIE._match_id(video_url)
            title = mobj.group('title')
            entries.append(self.url_result(
                video_url, ie=CustomMotherlessIE.ie_key(), video_id=video_id,
                video_title=title))
        ...
        if not entries:
            entries = [
                self.url_result(
                    ...
                    ie=CustomMotherlessIE.ie_key(), video_id=entry_id)
