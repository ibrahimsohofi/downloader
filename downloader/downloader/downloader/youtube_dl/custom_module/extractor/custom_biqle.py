...
from youtube_dl.extractor.vk import VKIE
from youtube_dl.compat import (
    compat_b64decode,
    compat_urllib_parse_unquote,
)
from youtube_dl.utils import int_or_none, urljoin, ExtractorError


class CustomBIQLEIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        embed_url_id = self._proto_relative_url(self._search_regex(
            r'id="data-embed-video-(.*?)"',
            webpage, 'embed url', default=None))
        if embed_url_id:
            embed_url = urljoin('https://daxab.com/player/', embed_url_id)
        else:
            raise ExtractorError('Can`t found embed url')
