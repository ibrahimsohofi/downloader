...
class CustomNaverBaseIE(InfoExtractor):
...
class CustomNaverIE(CustomNaverBaseIE):
    _VALID_URL = r'http://naver\.me/(?P<id>[^&]+)'
    _VALID_URL1 = r'https?://(?:m\.)?tv(?:cast)?\.naver\.(?:com|me)/(?:v|embed)/(?P<id>\d+)'
    _GEO_BYPASS = False
    _TESTS = [{
    ...
    def we_match_id(self, url):
        VALID_URL_RE = re.compile(self._VALID_URL1)
    ...
    def _real_extract(self, url):
        if '/v/' in url:
            video_id = self.we_match_id(url)
        else:
            video_id = self._match_id(url)
        content, info = self._download_webpage_handle(
            url,
            video_id,
        )
        if info.code == 200:
            url_re = re.search(
                r'<meta property="al:web:url" content="(.*?)">',
                content,
                re.S
            )
            if url_re:
