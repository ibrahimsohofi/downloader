...
from youtube_dl.extractor.theplatform import ThePlatformIE
...
class CustomNBCSportsVPlayerIE(InfoExtractor):
    ...
    def _extract_url(webpage):
        iframe_m = re.search(
            r'<iframe[^>]+src="(?P<url>https?://(?:vplayer\.|www\.)nbcsports\.com/[^"]+)"',
            webpage)
        if iframe_m:
            return iframe_m.group('url').replace('//www', '//vplayer')

    def _real_extract(self, url):
        video_id = self._match_id(url)
        webpage = self._download_webpage(url.replace('//vplayer', '//www'), video_id)
        theplatform_url = self._og_search_video_url(webpage).replace(
            'vplayer.nbcsports.com', 'player.theplatform.com')
        return self.url_result(theplatform_url, 'ThePlatform')


class CustomNBCSportsIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        return self.url_result(
            CustomNBCSportsVPlayerIE._extract_url(webpage), 'CustomNBCSportsVPlayer')


class CustomNBCSportsStreamIE(AdobePassIE):
...
class CustomCSNNEIE(InfoExtractor):
...
class CustomNBCNewsIE(ThePlatformIE):
...
class CustomNBCOlympicsIE(InfoExtractor):
    IE_NAME = 'nbcolympics'
...
class CustomNBCOlympicsStreamIE(AdobePassIE):
