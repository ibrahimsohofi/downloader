...
from .custom_youtube import CustomYoutubeIE


class CustomVKBaseIE(InfoExtractor):
    _NETRC_MACHINE = 'vk'
...
class CustomVKIE(CustomVKBaseIE):
    IE_NAME = 'vk'
    IE_DESC = 'VK'
    _VALID_URL = r'''(?x)
                    https?://
    ...
    def _real_extract(self, url):
        ...
        youtube_url = CustomYoutubeIE._extract_url(info_page)
        if youtube_url:
            return self.url_result(youtube_url, CustomYoutubeIE.ie_key())
        ...
        player_str = f'''<iframe width="100%" height="100%" frameborder="0" allowfullscreen src="https://vk.com/video_ext.php?oid={mv_data['oid']}&id={mv_data['vid']}&hash={player['params'][0]['embed_hash']}"></iframe>'''
...
class CustomVKUserVideosIE(CustomVKBaseIE):
    ...
    def _fetch_page(self, page_id, section, page):
        ...
        for video in l:
            ...
            info = {
                'title': video[3],
                'duration': duration,
                'url': 'http://vk.com/video' + video_id,
                'id': video_id,
                'ie_key': CustomVKIE.ie_key(),
                'thumbnail': video[2],
                'publishedAt': datetime.datetime.fromtimestamp(video[9]).strftime('%Y-%m-%d'),
                'player': ''
            }
            yield info
...
class CustomVKUserPageIE(CustomVKBaseIE):
    _VALID_URL = r'https?://(?:(?:m|new)\.)?vk\.com/(?P<user>[^/]{2,})/?(?:$|[?#])?'

    def _real_extract(self, url):
        user = re.match(self._VALID_URL, url).group('user')
        webpage = self._download_webpage(url, user)
        user_ = self._html_search_meta('og:url', webpage, 'image')
        if 'public' in user_:
            user_id = user_.split('public')[-1]
            user_id = f'-{user_id}'
        else:
            user_id = user_.split('id')[-1]
        return {
            '_type': 'url_transparent',
            'ie_key': CustomVKUserVideosIE.ie_key(),
            'url': f'https://vk.com/videos{user_id}',
        }


class CustomVKWallPostIE(CustomVKBaseIE):
    IE_NAME = 'vk:wallpost'
    ...
    def _real_extract(self, url):
        ...
        for video in re.finditer(
            ...
            entries.append(self.url_result(
                compat_urlparse.urljoin(url, video.group('url')), CustomVKIE.ie_key()))
...
class CustomVKMusicPlayListIE(CustomVKBaseIE):
    IE_NAME = 'vk:music playlist'
    '''https://vk.com/music?z=audio_playlist-71669171_85405520/b2fb1e509090dd232e'''
    '''https://vk.com/music/playlist/-71669171_85405520_b2fb1e509090dd232e'''
    _VALID_URL = r'https?://(?:(?:m|new)\.)?vk\.com/music/playlist/(?P<id>[^/?#&]+)'
    _VALID_URL1 = r'https?://(?:(?:m|new)\.)?vk\.com/music\?z=audio_playlist(?P<id>[^?#&]+)'

    def _download_payload(self, path, video_id, data, fatal=True):
        data['al'] = 1
        code, payload = self._download_json(
            'https://vk.com/%s.php?act=reload_audio' % path, video_id,
            data=urlencode_postdata(data), fatal=fatal,
            headers={'X-Requested-With': 'XMLHttpRequest'})['payload']
        if code == '3':
            self.raise_login_required()
    ...
    def _fetch_page(self, page_id, section, page):
        ids = '''-71669171_456246653_261bedacdb3aa691a7_c46a5055289885f8c8,-71669171_456246652_7dfe07aaca0ab9908a_8ea7d2aab39068307c,-71669171_456246651_d957d72ddaef456d3f_c4d61851fc6e23785a,-71669171_456246650_89e60f5da4d90ac85a_2356f8a93b808cc54c,-71669171_456246649_351bc9733d617cfb8a_abbcf5c0e7e654375b'''
        l = self._download_payload('al_audio', page_id, {
            'ids': ids,
        })[0][section]['list']
    ...
    def _real_extract(self, url):
        if re.match(self._VALID_URL1, url) is not None:
            sanitized_req = sanitized_Request(url)
            sanitized_response = self._request_webpage(
                sanitized_req, 'jump',
                note=False, errnote='Could not send HEAD request to %s' % url,
                fatal=False)

            if sanitized_response is not False:
                # Check for redirect
                new_url = sanitized_response.geturl()
                new_url = compat_urllib_parse_unquote(new_url)
                if re.match(self._VALID_URL, new_url) is not None:
                    url = new_url
                else:
                    raise ExtractorError(f'Unsupported URL: {sanitized_response.geturl()}')
            else:
                raise ExtractorError(f'Unsupported URL: {url}')
        ...
        entries = OnDemandPagedList(
            functools.partial(self._fetch_page, video_id, section),
            self._PAGE_SIZE)
