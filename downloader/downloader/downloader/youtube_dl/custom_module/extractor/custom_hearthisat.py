...
class CustomHearThisAtIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        duration = re.findall(r'data-length="(\d+)', webpage, re.S)
        if duration:
            duration = str_to_int(duration[-1])
        else:
            duration = 5
        ...
        if mp3_url:
            ...
            mp3_url = re.findall(r'data-mp3="(.*?)"', webpage, re.S)
            if mp3_url:
                mp3_url = mp3_url[-1]
            else:
                raise Exception('No Valid Mp3 Url To Download!')
        formats.append({
            'format_id': 'mp3',
            'vcodec': 'none',
            'acodec': 'mp3',
        ...
        formats = [x for x in formats if '/untitled/stream.mp3' not in x['url']]
