...
class CustomGameStarIE(InfoExtractor):
    _VALID_URL = r'https?://(?:www\.)?game(?P<site>pro|star)\.de/(?:videos|artikel)/.*,(?P<id>[0-9]+)\.html'
    _TESTS = [{
    ...
    def _real_extract(self, url):
        ...
        if '/artikel/' in url:
            hrefs = re.findall(
                r'<a href="(.*?)" class="js-lbx js-ga" data-category="lightbox" data-label="video" data-action="open"',
                webpage
            )
            if hrefs:
                hrefs = list(map(lambda x: urljoin('https://www.gamestar.de', x), hrefs))
                hrefs = list(filter(self.suitable, hrefs))
                if hrefs:
                    href = hrefs[0]
                    return self.url_result(href, 'CustomGameStar')
            else:
                raise
