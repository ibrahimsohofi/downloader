...
class CustomLyndaBaseIE(InfoExtractor):
...
class CustomLyndaIE(CustomLyndaBaseIE):
    IE_NAME = 'lynda'
...
class CustomLyndaCourseIE(CustomLyndaBaseIE):
    ...
    def _real_extract(self, url):
        ...
        if not course:
            ...
            entries = [
                self.url_result(
                    item_template % video_id, ie=CustomLyndaIE.ie_key(),
                    video_id=video_id)
