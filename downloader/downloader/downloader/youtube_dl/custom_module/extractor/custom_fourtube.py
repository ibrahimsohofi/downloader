...
class CustomFourTubeBaseIE(InfoExtractor):
    @classmethod
    ...
    def _extract_formats(self, url, video_id, media_id, sources):
        token_url = 'https://%s/%s/desktop/%s' % (
            self._TKN_HOST, media_id, '+'.join([s for s, _ in sources]))
        ...
        formats = [{
            'url': tokens[fmt]['token'],
            'format_id': fmt + 'p',
            'resolution': fmt + 'p',
            'quality': int(fmt),
            'filesize': (int(size.replace('MB', '')) * 1024 * 1024) if 'MB' in size else ((int(size.replace('GB', '')) * 1024 * 1024 * 1024) if 'GB' in size else '')
        } for (fmt, size) in sources]
    ...
    def _real_extract(self, url):
        ...
        sources = re.findall(
            r'<button id="download.+?p".+?>(.*?)</button>',
            webpage,
            re.S
        )
        if sources:
            sources = [tuple(map(lambda x: x.strip().replace('p', ''), s.split('•'))) for s in sources]
...
class CustomFourTubeIE(CustomFourTubeBaseIE):
    IE_NAME = 'custom 4tube'
...
class CustomFuxIE(CustomFourTubeBaseIE):
    IE_NAME = 'custom fux'
...
class CustomPornTubeIE(CustomFourTubeBaseIE):
    IE_NAME = 'custom porntube'
    ...
    def _real_extract(self, url):
        ...
        sources = [(compat_str(e['height']), '{}MB'.format(int(e['sizeInBytes'] / 1024 / 1024)) if len(compat_str(int(e['sizeInBytes'] / 1024 / 1024))) <= 3 else '{}GB'.format(int(e['sizeInBytes'] / 1024 / 1024 / 1024)))
        ...
        publishedAt = unified_timestamp(video.get('publishedAt'))
        publishedAt = datetime.datetime.fromtimestamp(publishedAt).strftime('%Y-%m-%d')
        ...
        return {
            ...
            'view_count': view_count,
            'duration': duration,
            'player': '<iframe width="100%" height="100%" frameborder="0" allowfullscreen src="https://www.porntube.com/embed/{}"></iframe>'.format(video_id),
            'age_limit': 18,
        }


class CustomPornerBrosIE(CustomFourTubeBaseIE):
