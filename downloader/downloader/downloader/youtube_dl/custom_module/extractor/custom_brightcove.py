...
class CustomBrightcoveLegacyIE(InfoExtractor):
    IE_NAME = 'custom:brightcove:legacy'
    ...
    def _real_extract(self, url):
        ...
        if videoPlayer:
            ...
            if publisher_id:
                ...
                return self.url_result(brightcove_new_url, CustomBrightcoveNewIE.ie_key(), video_id)
...
class CustomBrightcoveNewIE(InfoExtractor):
    IE_NAME = 'custom:brightcove:new'
    ...
    def suitable(cls, url):
        rs = re.match(cls._VALID_URL, url) is not None or 'brightcove.com/' in url
        return rs

    @staticmethod
    def _extract_url(ie, webpage):
        urls = CustomBrightcoveNewIE._extract_urls(ie, webpage)
        return urls[0] if urls else None
    ...
    def _real_extract(self, url):
        if 'brightcove.com/' in url:
            page = self._download_webpage(url, 'detail page')
            url = self._html_search_meta(r'twitter:player', page, 'real url', default='no link')
            if url == 'no link':
                raise ExtractorError('This link can`t found link!')
