...
class CustomXTubeIE(InfoExtractor):
    _VALID_URL = r'''(?x)
                        (?:
                            xtube:|
    ...
    def _real_extract(self, url):
        ...
        js = self._search_regex(
            r'playerConf=({.*?}),playerWrapper=', webpage, 'config',
            default='{}')
        js_json = js_to_json(js)
        js_json = js_json.replace(':!0', ': true').replace(':!1', ': false')
        config = json.loads(js_json)
        if config:
            ...
            if isinstance(config, dict):
                ...
                sources = config.get('sources') or config.get('format') or config.get('mediaDefinition')
            else:
                raise ExtractorError('Can`t not found js_json, webpage change!')
        else:
            raise ExtractorError('Can`t not found js_json, webpage change!')
        ...
        for source in sources:
            if source['videoUrl'] and source['format'] == 'mp4':
                formats.append({
                    'url': source['videoUrl'],
                    'format_id': source['videoUrl'],
                    'height': source['quality'],
                })
...
class CustomXTubeUserIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        for pagenum in itertools.count(1):
            ...
            for video_id in orderedSet([video_id for _, video_id in re.findall(
                    r'data-plid=(["\'])(.+?)\1', html)]):
                entries.append(self.url_result('xtube:%s' % video_id, CustomXTubeIE.ie_key()))
