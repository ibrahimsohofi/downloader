...
class CustomRaiBaseIE(InfoExtractor):
...
class CustomRaiPlayIE(CustomRaiBaseIE):
    _VALID_URL = r'(?P<base>https?://(?:www\.)?raiplay\.it/.+?-(?P<id>%s))\.(?:html|json)' % CustomRaiBaseIE._UUID_RE
    _TESTS = [{
...
class CustomRaiPlayLiveIE(CustomRaiPlayIE):
...
class CustomRaiPlayPlaylistIE(InfoExtractor):
        ...
        for b in (program.get('blocks') or []):
            for s in (b.get('sets') or []):
                ...
                for m in (medias.get('items') or []):
                    ...
                    if not path_id:
                        continue
                    thumbnail = urljoin(url, m['images']['landscape'])
                    video_url = urljoin(url, path_id.replace('.json', '.html'))
                    info = {
                        'title': m['name'],
                        'duration': m['duration'],
                        'url': video_url,
                        'id': m['id'],
                        'ie_key': CustomRaiPlayIE.ie_key(),
                        'thumbnail': thumbnail,
                        'player': f'<img src="{thumbnail}">'
                    }
                    entries.append(info)
...
class CustomRaiIE(CustomRaiBaseIE):
    _VALID_URL = r'https?://[^/]+\.(?:rai\.(?:it|tv)|rainews\.it)/.+?-(?P<id>%s)(?:-.+?)?\.html' % CustomRaiBaseIE._UUID_RE
    _TESTS = [{
