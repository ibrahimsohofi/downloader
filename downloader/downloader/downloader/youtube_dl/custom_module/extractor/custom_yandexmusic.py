...
class CustomYandexMusicBaseIE(InfoExtractor):
    @staticmethod
    def _handle_error(response):
        if isinstance(response, dict):
            ...
            if response.get('type') == 'captcha' or 'captcha' in response:
                CustomYandexMusicBaseIE._raise_captcha()
    ...
    def _download_webpage_handle(self, *args, **kwargs):
        webpage = super(CustomYandexMusicBaseIE, self)._download_webpage_handle(*args, **kwargs)
        if 'Нам очень жаль, но&nbsp;запросы, поступившие с&nbsp;вашего IP-адреса, похожи на&nbsp;автоматические.' in webpage:
            self._raise_captcha()
        return webpage

    def _download_json(self, *args, **kwargs):
        response = super(CustomYandexMusicBaseIE, self)._download_json(*args, **kwargs)
...
class CustomYandexMusicTrackIE(CustomYandexMusicBaseIE):
    ...
    def _real_extract(self, url):
        ...
        track_info = {
            ...
            'player': f'<iframe width="100%" height="20%" frameborder="0" allowfullscreen src="https://music.yandex.ru/iframe/#track/{track_id}"></iframe>'
        }
        ...
        track_info['formats'] = formats
        return track_info


class CustomYandexMusicPlaylistBaseIE(CustomYandexMusicBaseIE):
    def _extract_tracks(self, source, item_id, url, tld):
        ...
        if len(tracks) < len(track_ids):
            ...
            group_num = (len(missing_track_ids) // 290) + 1
            split_missing_track_ids = [missing_track_ids[n * 290: (n + 1) * 290] for n in range(group_num)]
            for part_missing_track_ids in split_missing_track_ids:
                missing_tracks = self._call_api(
                    'track-entries', tld, url, item_id,
                    'Downloading missing tracks JSON', {
                        'entries': ','.join(part_missing_track_ids),
                        'lang': tld,
    ...
    def _build_playlist(self, tracks):
        ...
        for track in tracks:
            ...
            if not album_id:
                continue
            thumbnail = (track.get('coverUri') or track.get('ogImage', '')).replace('%%', 'orig')
            if thumbnail and not thumbnail.startswith('http'):
                thumbnail = 'http://' + thumbnail
            info = {
                'id': track_id,
                'title': track.get('title', '') or track.get('type', ''),
                'publishedAt': '',
                'thumbnail': thumbnail,
                'description': track.get('title') or track.get('type', ''),
...
class CustomYandexMusicAlbumIE(CustomYandexMusicPlaylistBaseIE):
...
class CustomYandexMusicPlaylistIE(CustomYandexMusicPlaylistBaseIE):
...
class CustomYandexMusicArtistBaseIE(CustomYandexMusicPlaylistBaseIE):
...
class CustomYandexMusicArtistTracksIE(CustomYandexMusicArtistBaseIE):
...
class CustomYandexMusicArtistAlbumsIE(CustomYandexMusicArtistBaseIE):
    ...
    def _real_extract(self, url):
        ...
        for album in data['albums']:
            ...
            entries.append(self.url_result(
                ...
                ie=CustomYandexMusicAlbumIE.ie_key(), video_id=album_id))
        artist = try_get(data, lambda x: x['artist']['name'], compat_str)
