...
class CustomSpankBangIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        formats = list(filter(lambda x: 'hls-' not in x['format_id'], formats))
        ...
        ) or datetime.datetime.fromtimestamp(info['timestamp']).strftime('%Y-%m-%d')
        publishedAt = f'Published on {date}'
...
class CustomSpankBangPlaylistIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        entries = [self.url_result(
            ...
            ie=CustomSpankBangIE.ie_key(), video_id=video_id)
