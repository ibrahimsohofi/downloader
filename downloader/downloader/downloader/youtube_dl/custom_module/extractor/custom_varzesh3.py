...
class CustomVarzesh3IE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        embed_url = self._html_search_meta(
            ['og:video:url', 'og:video:secure_url'], webpage, 'embed_url'
        )

        videos = re.findall(
            r'<a class="dropdown-item".*?</a>', webpage, re.S
        )

        formats = []
        for video_str in videos:
            video_url = self._html_search_regex(
                r'''href="(.*?)"''', video_str, 'mp4_url'
            )
            height_str = self._html_search_regex(
                r'''<span>(.*?)</span>''', video_str, 'height'
            )
            ...
            if not formats:
                m3u8_str = self._html_search_regex(
                    r'data-m3u8="(.*?)"', webpage, 'm3u8_url', default=''
                )
                if m3u8_str:
                    formats.append(
                        {
                    ...
                    raise ExtractorError('Not Found Video Url!')
