...
class CustomWistiaIE(InfoExtractor):
    ...
    def _extract_url(webpage):
        urls = CustomWistiaIE._extract_urls(webpage)
    ...
    def suitable(cls, url):
        rs = re.match(cls._VALID_URL, url) is not None or ('wvideo=' in url or 'wvideoid=' in url)
        return rs

    def _real_extract(self, url):
        if 'wvideo=' in url or 'wvideoid=' in url:
            if 'wvideo=' in url:
                mobj = re.search(r'wvideo=([a-z0-9]{10})', url, re.S)
            else:
                mobj = re.search(r'wvideoid=([a-z0-9]{10})', url, re.S)
            if not mobj:
                raise
